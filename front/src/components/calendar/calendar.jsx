            ...
            <div className={styles.calendar_wrap}>
                <div className={styles.year}>
                    ...
                    <div className={styles.year_btt}>
                        <button onClick={()=>{prevMonth()}}><FontAwesomeIcon icon={faAngleLeft} /></button>
                        <button onClick={()=>{nextMonth()}}><FontAwesomeIcon icon={faAngleRight} /></button>
                    </div>
                </div>
                                    ...
                                    <tr key={index}>
                                        {
                                            item.map((item2,index2)=>{
                                                return(
                                                    <td key={index2} 
                                                    className={item2 == today.date && today.month == selectedMonth ? `${styles.active}` : null} 
                                                    onClick={(e)=>{setDateText([selectedYear,selectedMonth,Number(e.target.textContent)]);}}><em>{item[index2]}</em></td>
                                                )
                                            })
                                        }
                                    </tr>
